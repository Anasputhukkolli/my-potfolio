...
import { ExternalLink, Github, Figma, Play, Image as ImageIcon, Loader2 } from "lucide-react";
    ...
    <article className="min-h-screen bg-white px-6 max-w-5xl mx-auto"
      ...
      <style>{`
        ...
        .port-card {
          ...
          border: 2.5px solid #111; border-radius: 18px;
          overflow: hidden; box-shadow: 5px 5px 0 #111;
          background: #fff;
          transition: transform .18s cubic-bezier(.34,1.56,.64,1), box-shadow .18s;
          animation: fade-up .4s ease both;
        }
        .port-card:hover { transform: translate(-3px,-3px); box-shadow: 8px 8px 0 #111; }
        .port-tag {
          font-family: 'Fredoka One', cursive; font-size: 11px;
          background: #FFE033; border: 2px solid #111;
          border-radius: 8px; padding: 3px 9px; color: #111; box-shadow: 2px 2px 0 #111;
        }
        .filter-btn {
          font-family: 'Fredoka One', cursive; font-size: 12px;
          letter-spacing: .04em; padding: 7px 16px;
          border-radius: 10px; border: 2.5px solid #111;
          cursor: pointer;
          transition: transform .15s, box-shadow .15s, background .15s;
          box-shadow: 3px 3px 0 #111;
        }
        .filter-btn:hover { transform: translate(-1px,-1px); box-shadow: 4px 4px 0 #111; }
        .connect-card {
          display: flex; align-items: center; gap: 12px;
          border: 2.5px solid #111; border-radius: 14px;
          padding: 14px 18px; box-shadow: 4px 4px 0 #111;
          background: #fff; text-decoration: none;
          transition: transform .18s cubic-bezier(.34,1.56,.64,1), box-shadow .18s, background .18s;
        }
        .connect-card:hover { transform: translate(-2px,-2px); box-shadow: 6px 6px 0 #111; background: #FFE033; }
        .link-btn {
          display: inline-flex; align-items: center; gap: 6px;
          font-family: 'Fredoka One', cursive; font-size: 12px;
          color: #111; text-decoration: none; background: #fff;
          border: 2px solid #111; border-radius: 8px;
          padding: 4px 10px; box-shadow: 2px 2px 0 #111;
          transition: background .15s, transform .15s, box-shadow .15s;
        }
        .link-btn:hover { background: #FFE033; transform: translate(-1px,-1px); box-shadow: 3px 3px 0 #111; }
      `}</style>
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, marginBottom: 24 }}>
          {categories.map((cat) => (
            <button
              ...
              style={{
                background: activeCategory === cat ? '#FFE033' : '#fff',
      ...
      {loading && (
        <div style={{ display: 'flex', justifyContent: 'center', padding: '60px 0' }}>
          <Loader2 size={28} style={{ animation: 'spin 1s linear infinite', color: '#aaa' }} />
        </div>
      )}

      {/* Error */}
      {error && (
        <div style={{ padding: '40px 0', textAlign: 'center', ...fredoka, color: '#e55', fontSize: 14 }}>
          {error}
        </div>
      )}
              ...
              <div key={project.id} className="port-card" style={{ animationDelay: `${i * 0.08}s` }}>
                <div style={{ position: 'relative', height: 180, overflow: 'hidden', borderBottom: '2.5px solid #111', background: '#f5f5f5' }}>
                  ...
                  <span style={{
                    ...
                    boxShadow: '2px 2px 0 #111', color: '#111', letterSpacing: '.06em',
                  }}>
                    {project.category}
                  </span>
                </div>
